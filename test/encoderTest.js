...
describe('URI Encoder', function () {
    ...
    describe('should substitute known protocols', function () {
        ...
        it('urn:uuid:', function () {
            var encoded = uriBeacon.encode("urn:uuid:B1E13D51-5FC9-4D5B-902B-AB668DD54981");
    ...
    describe('should handle unknown protocols', function () {
        ...
        it('tel', function () {
            ...
            var encoded = uriBeacon.encode(uri);
    ...
    describe('should substitute known domains', function () {
        ...
        it('net', function () {
            var encoded = uriBeacon.encode("foo.net");
            assert.equal(10, encoded[3]); // domain
        });

        it('info', function () {
            var encoded = uriBeacon.encode("foo.info");
            assert.equal(11, encoded[3]); // domain
        });

        it('biz', function () {
            var encoded = uriBeacon.encode("foo.biz");
            assert.equal(12, encoded[3]); // domain
        });

        it('gov', function () {
            var encoded = uriBeacon.encode("foo.gov");
    ...
    it('should handle trailing slash', function () {
        ...
        // no trailing slash
        var encoded = uriBeacon.encode("http://www.chariotsolutions.com/");
        assert.equal(0, encoded[0]); // prefix
        assert.equal(0, encoded[17]); // domain
        assert.equal(18, encoded.length);

        // trailing slash
        encoded = uriBeacon.encode("http://www.chariotsolutions.com/");
        assert.equal(0, encoded[0]); // prefix
        assert.equal(0, encoded[17]); // domain
        assert.equal(18, encoded.length);
    ...
    describe('should encode UUIDs', function() {
        ...
        it('without dashes', function() {
            var encoded = uriBeacon.encode("urn:uuid:B1E13D515FC94D5B902BAB668DD54981");
            assert.equal(4, encoded[0]); // prefix
            assert.equal(17, encoded.length);
        });

        it('too short', function() {
            var encoded = uriBeacon.encode("urn:uuid:B1E13D51");
            assert.equal(4, encoded[0]); // prefix
        ...
        it('too long', function() {
            var encoded = uriBeacon.encode("urn:uuid:B1E13D51-5FC9-4D5B-902B-AB668DD54981999999999999999999999999");
            assert.equal(4, encoded[0]); // prefix
            assert.equal(17, encoded.length);
        });
